...
describe('resource advisor', () => {
  ...
  test('chat: request options instead of params are passed correctly', async () => {
  ...
  test('retrieveHistory: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.ai.advisor.retrieveHistory({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Jocall3.NotFoundError,
    );
  });
